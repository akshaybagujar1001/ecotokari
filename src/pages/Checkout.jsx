  ...
  if (placed) {
    return (
      <div className="min-h-screen flex flex-col items-center justify-center px-4 text-center">
        ...
        <h1 className="font-display text-3xl font-bold text-brand-950 mb-3">Order Placed!</h1>
        <p className="text-gray-500 max-w-md mb-2">
          Your order has been received. You can track the status from your account page.
        </p>
  ...
  return (
    <div className="min-h-screen py-10">
      <div className="page-shell max-w-4xl">
        <h1 className="font-display text-3xl font-bold text-brand-950 mb-8">Checkout</h1>
            ...
            <div key={s} className="flex items-center flex-1">
              <div
                className={`w-8 h-8 rounded-full flex items-center justify-center text-sm font-bold transition-colors ${
                  i < step ? 'bg-brand-600 text-white' : i === step ? 'bg-brand-700 text-white' : 'bg-gray-200 text-gray-500'
                }`}
              >
                {i < step ? <Check size={14} /> : i + 1}
              </div>
              <span className={`ml-2 text-sm font-medium ${i === step ? 'text-brand-950' : 'text-gray-400'}`}>{s}</span>
              {i < STEPS.length - 1 && (
                <div className={`flex-1 h-px mx-3 ${i < step ? 'bg-brand-600' : 'bg-gray-200'}`} />
              )}
            </div>
          ))}
        </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
          <div className="lg:col-span-2">
            {step === 0 && (
              <div className="surface p-6">
                <h2 className="font-semibold text-brand-950 text-lg mb-5">Shipping Details</h2>
            ...
            {step === 1 && (
              <div className="surface p-6">
                <h2 className="font-semibold text-brand-950 text-lg mb-5">Payment Method</h2>
                  ...
                  {[
                    ...
                    <label
                      ...
                      className={`flex items-center gap-4 p-4 rounded-xl border-2 cursor-pointer transition-colors ${
                        payment === m.id ? 'border-brand-600 bg-brand-50' : 'border-gray-200 hover:border-gray-300'
                      }`}
                    >
                      <input
                        type="radio"
                      ...
                      <div>
                        <p className="font-medium text-brand-950">{m.label}</p>
                        <p className="text-xs text-gray-400">{m.sub}</p>
                      </div>
                      <div
                        className={`ml-auto w-5 h-5 rounded-full border-2 flex items-center justify-center ${
                          payment === m.id ? 'border-brand-600 bg-brand-600' : 'border-gray-300'
                        }`}
                      >
            ...
            {step === 2 && (
              <div className="surface p-6">
                <h2 className="font-semibold text-brand-950 text-lg mb-5">Review and Confirm</h2>
                  ...
                  {items.map(i => (
                    <div key={i.id} className="flex items-center gap-3">
                      {i.image ? (
                        ...
                        <div className="flex h-12 w-12 items-center justify-center rounded-lg bg-brand-100 font-bold text-brand-400">
                          {i.name[0]}
                        </div>
                      )}
                      <div className="flex-1">
                        <p className="text-sm font-medium text-brand-950">{i.name}</p>
                        <p className="text-xs text-gray-400">Qty: {i.qty}</p>
                      </div>
                      <span className="font-semibold text-brand-950">{'\u20B9'}{i.price * i.qty}</span>
                    </div>
                  ))}
                </div>
                <div className="bg-brand-50/70 rounded-xl p-4 text-sm space-y-1 mb-6">
          ...
          <div>
            <div className="surface p-5 sticky top-24 text-sm">
              <h3 className="font-semibold text-brand-950 mb-4">Order Summary</h3>
              {items.map(i => (
                <div key={i.id} className="flex justify-between py-1.5 border-b border-brand-50">
                  <span className="text-gray-600 truncate mr-2">{i.name} x {i.qty}</span>
                  <span className="font-medium shrink-0">{'\u20B9'}{i.price * i.qty}</span>
                </div>
              ))}
              ...
              <div className="border-t border-brand-100 mt-3 pt-3 flex justify-between font-bold text-brand-950 text-base">
